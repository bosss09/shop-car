                                                 ...
                                                 }) =>
  {
    ...
    addComponents(
      _.fromPairs(_.map(options.themes, (theme, themeName) => [
        ...
        ))), (value, key) => [[`--fort-${e(key)}`, value], [`--fort-${e(key)}-rgb`, chroma(value).rgb().join(',')]]))),
      ])),
    );
    ...
    const schemeCustomProps = _.map(['light', 'dark'], (colorScheme) =>
    {
      ...
      const background = theme(`fort.customProps.background.${colorScheme}`);
      const foreground = theme(`fort.customProps.foreground.${colorScheme}`);
      ...
      return {
        [(isDark ? darkSchemeSelectors : lightSchemeSelectors)]: {
          ...
          ..._.fromPairs(_.flatten(_.map(background, (value, key) => [[`--fort-${e(key)}`, value], [`--fort-${e(key)}-rgb`, chroma(value).rgb().join(',')]]))),
          ..._.fromPairs(_.flatten(_.map(foreground, (value, key) => [[`--fort-${e(key)}`, value], [`--fort-${e(key)}-rgb`, chroma(value).rgb().join(',')]]))),
        },
      };
    });
      ...
      theme: {
        extend: {
          ...
          colors: _.fromPairs(_.flatten(_.map(_.keys(flattenColorPalette(normalizeTheme(options.themes.default))), (name) => [
            [name, `rgba(var(--fort-${name}-rgb), <alpha-value>)`],
            [`on-${name}`, `rgba(var(--fort-on-${name}-rgb), <alpha-value>)`],
          ]))),
        },
        fort  : {
          customProps: {
