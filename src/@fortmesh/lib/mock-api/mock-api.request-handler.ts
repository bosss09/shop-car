...
import { FortMockApiReplyCallback } from '@fortmesh/lib/mock-api/mock-api.types';
import { Observable, of, take, throwError } from 'rxjs';

export class FortMockApiHandler
{
  request!: HttpRequest<any>;
  urlParams!: { [key: string]: string };

  // Private
  private _reply: FortMockApiReplyCallback = undefined;
  ...
  get response(): Observable<any>
  {
    ...
    if ( this._replyCount > 0 && this._replyCount <= this._replied )
    {
      ...
      // New private Function Error
      return this.processError('Execution limit has been reached!');
    }
    ...
    if ( !this._reply )
    {
      ...
      // New private Function Error
      return this.processError('Response callback function does not exist!');
    }
    ...
    if ( !this.request )
    {
      ...
      // New private Function Error
      return this.processError('Request does not exist!');
    }
   ...
   * @param callback
   */
  reply(callback: FortMockApiReplyCallback): void
  {
    // Store the reply
