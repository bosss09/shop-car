...
import { FORT_MOCK_API_DEFAULT_DELAY } from '@fortmesh/lib/mock-api/mock-api.constants';
import { FortMockApiService } from '@fortmesh/lib/mock-api/mock-api.service';
import { delay, Observable, of, switchMap, throwError } from 'rxjs';

export const mockApiInterceptor = (request: HttpRequest<unknown>, next: HttpHandlerFn): Observable<HttpEvent<unknown>> =>
{
  const defaultDelay = inject(FORT_MOCK_API_DEFAULT_DELAY);
  const fortMockApiService = inject(FortMockApiService);

  // Try to get the request handler
  const {
    handler,
    urlParams,
  } = fortMockApiService.findHandler(request.method.toUpperCase(), request.url);
