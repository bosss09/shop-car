...
import { FortConfigService } from '@fortmesh/services/config';
import { fromPairs } from 'lodash-es';
import { map, Observable, ReplaySubject, switchMap } from 'rxjs';

@Injectable({providedIn: 'root'})
export class FortMediaWatcherService
{
  ...
  constructor(
    ...
    private _fortConfigService: FortConfigService,
  )
  {
    this._fortConfigService.config$.pipe(
      map(config => fromPairs(Object.entries(config.screens).map(([alias, screen]) => ([alias, `(min-width: ${screen})`])))),
      switchMap(screens => this._breakpointObserver.observe(Object.values(screens) as any).pipe(
      /*switchMap(screens => this._breakpointObserver.observe('large').pipe(*/
        map((state) =>
        {
