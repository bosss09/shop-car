...
import { ScrollbarGeometry, ScrollbarPosition } from '@fortmesh/directives/scrollbar/scrollbar.types';
import { merge } from 'lodash-es';
import PerfectScrollbar from 'perfect-scrollbar';
import { debounceTime, fromEvent, Subject, takeUntil } from 'rxjs';

@Directive({
  selector: '[fortScrollbar]',
  exportAs  : 'fortScrollReset',
  standalone: true
})
export class FortScrollbarDirective implements OnChanges, OnInit, OnDestroy
{

  /* eslint-disable @typescript-eslint/naming-convention */
  static ngAcceptInputType_fortScrollbar: BooleanInput;
  /* eslint-enable @typescript-eslint/naming-convention */

  @Input() fortScrollbar: boolean = true;
  @Input() fortScrollbarOptions: PerfectScrollbar.Options;
  ...
  {
    // Enabled
    if ( 'fortScrollbar' in changes )
    {
      // Interpret empty string as 'true'
      this.fortScrollbar = coerceBooleanProperty(changes.fortScrollbar.currentValue);

      // If enabled, init the directive
      if ( this.fortScrollbar )
      {
        this._init();
      }
    ...
    // Scrollbar options
    if ( 'fortScrollbarOptions' in changes )
    {
      // Merge the options
      this._options = merge({}, this._options, changes.fortScrollbarOptions.currentValue);
